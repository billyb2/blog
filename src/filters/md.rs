...
pub fn add_metadata(
	_md: &str, _path: &str, title: &str, author: &str, date: &str,
) -> (String, WriteTo) {
	(
		format!("## {title}\n### {date}\n### {author}"),
		WriteTo::Beginning,
...
pub fn apply_md_filters(
	md: &mut String, path: &str, title: &str, author: &str, date: &str,
	filters: &[fn(&str, &str, &str, &str, &str) -> (String, WriteTo)],
) {
	let (beginning, end): (Vec<_>, Vec<_>) = filters
		.iter()
		.map(|filter| filter(md, path, title, author, date))
